...
Node* DLList::_find(int index){
	...
	Node* cur = head; //variable to hold pointer to current node
...
void DLList::insert(int index, const int& item){
	...
	if(index == 0){ //if inserting at beginning
		...
		Node* prev = _find(index -1); //find node before insert location
		Node* newNode = new Node(item,prev->next_,prev); //create new node with each appropriate parameter
...
int DLList::_delete(int index){
	
	Node* nodeToDelete = _find(index);
...
DLList::DLList(const DLList& from)
...
void DLList::allocCopy(const DLList& from){
	...
	Node* cur = from.head; //pointer to current node, starting at from's head
	head = new Node(cur->item_); //create new node pointed to by head and copy to it
	
	Node* mycur = head; //pointer to this object's current node
	for (int i=0;i<from.size-1;i++){ //loop over nodes
		cur = cur->next_; //advance curr
		mycur->next_ = new Node(cur->item_,NULL,mycur); 
		mycur = mycur->next_;
		size++;
	}
	tail = mycur;
...
void DLList::append(const int& item){
	// post: append the item into the current list
	Node* newNode = new Node(item,NULL,tail); //create new node with prev of tail
...
void DLList::dealloc(){
	...
	Node* cur = head; //pointer to the current node, starting from head
